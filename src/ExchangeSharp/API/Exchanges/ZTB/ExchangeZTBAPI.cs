...
namespace ExchangeSharp
{
	...
	public partial class ExchangeZTBAPI : ExchangeAPI
	{
		string fastestUrl = null;
		int failedUrlCount;
		int successUrlCount;

		public override string BaseUrl { get; set; } = "https://www.ztb.im/api/v1";
		public override string BaseUrlWebSocket { get; set; } = "wss://ws.ztb.im/ws";
		int websocketMessageId = 0;
		string timeWindow;
		TaskCompletionSource<int> inited = new TaskCompletionSource<int>();

		public ExchangeZTBAPI()
		{
			...
			RateLimit = new RateGate(20, TimeSpan.FromSeconds(1));
			GetFastestUrl();
		}

		void GetFastestUrl()
		{
			fastestUrl = BaseUrlWebSocket;
			inited.SetResult(0);
		}
		...
		{
			if (result.Type == JTokenType.Object && (int)result["code"] != 0)
			{
		...
		{
			var symbol = (string)x["symbol"];
			return new ExchangeMarket
			{
				IsActive = (string)x["status"] == "trading",
				MarketSymbol = symbol,
				BaseCurrency = (string)x["baseAsset"],
				QuoteCurrency = (string)x["quoteAsset"],
				PriceStepSize = new decimal(1, 0, 0, false, (byte)x["quoteAssetPrecision"]),
				QuantityStepSize = new decimal(1, 0, 0, false, (byte)x["baseAssetPrecision"]),
		...
		protected override async Task<IWebSocket> OnGetTradesWebSocketAsync(Func<KeyValuePair<string, ExchangeTrade>, Task> callback, params string[] marketSymbols)
		{
			...
			return await ConnectWebSocketAsync(string.Empty, async (_socket, msg) =>
			{
				...
				if (token["method"].ToStringLowerInvariant() == "deals.update")
				{
					var args = token["params"];
					var symbol = args[0].ToStringUpperInvariant();
					var trades = args[1];
			...
			async (_socket2) =>
			{
				...
				await _socket2.SendMessageAsync(new { id, method = "deals.subscribe", @params = marketSymbols });
			});
		}
		...
		protected override async Task<IWebSocket> OnGetDeltaOrderBookWebSocketAsync(Action<ExchangeOrderBook> callback, int maxCount = 20, params string[] marketSymbols)
		{
			...
			return await ConnectWebSocketAsync(string.Empty, (_socket, msg) =>
			{
				...
				if (token["method"].ToStringLowerInvariant() == "depth.update")
				{
					...
					book.MaxCount = maxCount;
					book.IsFull = (bool)args[0];
					callback(book);
				}
				else if (token["error"].Type == JTokenType.Object)
				{
					return Task.FromException(new APIException($"OnGetDeltaOrderBookWebSocketAsync: {token}"));
				}
			...
			}, async (_socket) =>
			{
				...
				var args = new Object[]
				{
					string.Join(",", marketSymbols),
					maxCount,
					"0",
				};
				await _socket.SendMessageAsync(new { id, method = "depth.subscribe", @params = args});
			});
		}
	...
	public partial class ExchangeName { public const string ZTB = "ZTB"; }
